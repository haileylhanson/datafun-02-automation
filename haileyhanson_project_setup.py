"""
File: haileyhanson_project_setup.py
...
Author: Hailey Hanson - modified from outline by Denise Case
...
def create_folders_for_range(start_year: int, end_year: int) -> None:
    ...
    for year in range(start_year, end_year+1):
        year_path :str = str(ROOT_DIR) + "/" + str(year)
...
def create_folders_from_list(folder_list: list) -> None:
    ...
    for folder in folder_list:
        folder = folder.replace(" ", "") #remove any spaces
        folder = folder.lower() #set whole string to lowercase
...
def create_prefixed_folders_using_list_comprehension(folder_list: list, prefix: str) -> None:
    ...
    for folder in folder_list:
        new_path :str = str(ROOT_DIR) + "/" + prefix + str(folder) 
        os.makedirs(new_path, exist_ok=True)
        logger.info(f"Created folder: {new_path}") 
...
def main() -> None:
    ...
    logger.info(f"Byline: {utils_haileyhanson.get_byline()}")

    # Call function 1 to create folders for a range (e.g. years)
    create_folders_for_range(start_year=2020, end_year=2023)

    # Call function 2 to create folders given a list
    folder_names = ['data- Csv', 'data-excel', 'data-json'] #edited data-csv to test space & lowercase rules
